...
export interface CreateReleaseRequest {
  ...
  /** 滾動完成判斷條件 1：最少百分之 N 的容器創建；數值範圍（1-100） */
  min_created_percentage?: number;
  /** 滾動完成判斷條件 2：最少百分之 N 的容器啓動完成；數值範圍（1-100） */
...
export interface CreateTicketRequest {
  /** approved user. 審覈人 */
  approved_by?: string;
  /** type of approved user. 審覈用戶類型。Enums: "person_account","service_account" */
  approved_by_usertype?: string;
  /** release cluster, use default cluster when without it. 發佈的集羣, 不填則爲默認集羣 */
  cluster?: string;
  /** ID of used code revision, lower priority than use_latest_code_revision. 代碼版本 ID, 用指定代碼版本進行發佈 */
  code_revision_id?: string;
  /** description of this release. 發佈描述 */
  description?: string;
  /** release region. 發佈的 region */
  region: string;
  /** release type, enums: runtime_update、runtime_release. 發佈類型 */
  release_type?: string;
  /** replica limit. 實例數，只用作第一次發佈時需要。 */
  replica_limit?: Record<string, common.EmptyObject>;
  /** ID of revision, only works when rollback is true, lower priority than code_revision_id. 版本 ID, 回滾至某一個 revision */
  revision_id?: string;
  /** create ticket of rollback action. 回滾 */
  rollback?: boolean;
  /** rolling step. 滾動比例 */
  rolling_step?: number;
  /** ID of service */
  service_id: string;
  /** use latest code revision. 使用最新的代碼版本進行發佈 */
  use_latest_code_revision?: boolean;
  /** grey mqevent config. 灰度觸發器配置 */
  grey_mqevent_config?: Array<common.GreyMQEvent>;
  /** the code config. 發佈的代碼配置 */
  code_source?: string;
  /** the mqevent release type, enums: hot_load, deployment_rolling. 觸發器發佈類型配置 */
