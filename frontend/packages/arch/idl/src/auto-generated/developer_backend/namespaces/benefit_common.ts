...
這裏指的是Coze的AccountID */
  ...
  /** 優先取此 SpaceID，若未填充則兜底從 RelationBasic 中取 */
  space_id?: Int64;
  /** 資源消耗明細
資源用量上報時必填。保證冪等（確保資源消耗可重入），應在各資源層唯一 */
  uuid?: string;
  /** 模型資源 */
...
export interface CommonCheckAndUpdateResp {
  ...
  /** 權益校驗結果明細 */
  BenefitResult?: Partial<Record<BenefitType, CheckResultType>>;
  /** 用於串聯一次完整請求，如果返回值不爲""，調用方將該值置於ctx.COZE_RECORD_ROOT_ID中 */
  RootID?: string;
  BaseResp?: base.BaseResp;
}
...
/** 資源消耗通用消息體，資源上報上報時消息體需包含該結構。建議優先使用 CheckAndUpdateReq 結構，若自定義結構則加上此結構 */
