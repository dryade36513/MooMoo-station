/*
 * Copyright 2025 coze-dev Authors
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

/* eslint-disable */
/* tslint:disable */
// @ts-nocheck

export type Int64 = string | number;

export enum MetaType {
  /** 事件記憶元信息 */
  EventMeta = 1,
  /** 畫像記憶元信息 */
  ProfileMeta = 2,
}

export interface LongTermMemoryClearAllRequest {
  bot_id: string;
  connector_id: string;
  /** 僅舊鏈路xmemory使用  1: 原始對話 2: 總結後的話題 3: 精華記憶 */
  time_capsule_item_type?: number;
}

export interface LongTermMemoryClearAllResponse {}

export interface LongTermMemoryDeleteRequest {
  bot_id: string;
  connector_id: string;
  biz_ids: Array<string>;
  /** xmemory使用  1: 原始對話 2: 總結後的話題 3: 精華記憶 */
  time_capsule_item_type?: number;
  /** xmemory使用(如果是火山記憶就是其記憶id) */
  iids?: Array<string>;
  /** 火山記憶具體類型，key:iids, val: meta_type */
  memory_meta_type_map?: Record<string, MetaType>;
}

export interface LongTermMemoryDeleteResponse {}

export interface LongTermMemoryItem {
  /** 業務id 火山側的mempryID、xmemory側的BizId */
  biz_id: string;
  /** 事件文本 */
  text: string;
  /** 事件時間（時間戳） */
  event_time: string;
  /** xmemory側的記憶擴展 */
  ext?: Record<string, string>;
  /** xmemory記憶標籤 */
  tags?: Array<string>;
  /** xmemory記憶類型  1: 原始對話 2: 總結後的話題 3: 精華記憶 */
  time_capsule_item_type?: number;
  /** xmemory記憶的Iid */
  iid?: string;
  /** 火山記憶元數據類型 */
  meta_type?: MetaType;
  /** 火山記憶具體類型 */
  memory_type?: string;
}

export interface LongTermMemoryListRequest {
  bot_id: string;
  connector_id: string;
  /** offset、limit僅舊鏈路xmemory使用，火山側沒有分頁 */
  offset?: number;
  limit?: number;
  /** 僅舊鏈路xmemory使用  1: 原始對話 2: 總結後的話題 3: 精華記憶 */
  time_capsule_item_type?: number;
}

export interface LongTermMemoryListResponse {
  time_capsule_items: Array<LongTermMemoryItem>;
  total: number;
  /** 最近一次清空的時間戳 */
  last_clear_all_time?: string;
}

export interface LongTermMemoryUpdateRequest {
  bot_id: string;
  connector_id: string;
  biz_id: string;
  new_content: string;
  /** 事件時間（時間戳） */
  event_ms?: string;
  /** xmemory側的記憶擴展 */
  ext?: Record<string, string>;
  /** xmemory記憶標籤 */
  tags?: Array<string>;
  /** xmemory記憶類型  1: 原始對話 2: 總結後的話題 3: 精華記憶 */
  time_capsule_item_type?: number;
  /** xmemory記憶的Iid */
  iid?: string;
  /** 火山記憶元數據類型 */
  meta_type?: MetaType;
  /** 火山記憶具體類型 */
  memory_type?: string;
}

export interface LongTermMemoryUpdateResponse {}

export interface LongTermMemoryVersionRequest {
  bot_id: string;
}

export interface LongTermMemoryVersionResponse {
  /** 是否走Mars長期記憶 */
  MarsLongTermMemory: boolean;
}
/* eslint-enable */
