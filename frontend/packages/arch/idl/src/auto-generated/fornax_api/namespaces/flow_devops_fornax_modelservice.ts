...
export interface ExportSftTaskOutputToProviderRequest {
  ...
  /** provider=merlin時必傳，必須傳未過期的用戶的token */
  jwtToken?: string;
}

export interface ExportSftTaskOutputToProviderResponse {
  /** 任務產出在導出後的屬性，此接口已改爲異步，這個屬性已棄用 */
...
export interface GetSftTaskFoundationModelConfigRequest {
  ...
  providerType?: model.Provider;
  /** Provider爲merlin時傳vendor/model_name */
...
export interface GetSftTaskRequest {
  ...
  accountID?: string;
  taskID?: string;
  /** 用於JwtToken鑑權，這種情況下spaceID和accountID在鑑權後獲得 */
  Authorization?: string;
}
...
export interface GetUserProviderAuthStatusResponse {
  /** 是否已授權，當剩餘時長小於一週時，會被置爲未授權，此時需要用戶再次授權 */
...
export interface SearchByteTreeNodesRequest {
  ...
  /** 搜索父節點下滿足條件的子孫節點 */
  inherit?: boolean;
  /** 根據節點包含的 resource provider 過濾，僅葉節點有 provider 信息 */
...
export interface UpsertModelAndAccountRequest {
  modelAccount?: ModelAccount;
  /** 從精調任務新創建 merlin 模型時必傳，用於調用 Bernard 服務 */
  'x-jwt-token'?: string;
}
