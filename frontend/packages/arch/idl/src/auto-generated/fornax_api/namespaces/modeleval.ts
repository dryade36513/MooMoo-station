...
export interface OfflineEvalTaskCkptResult {
  ...
  /** 推理結果保存的hdfs地址，可能爲文件夾(開源模型必須時文件而非文件夾) */
  ...
  /** 批量推理結果的列名，老數據可能爲空，爲空時該值取output（parquet中的列名。如果是jsonl，格式是方舟固定的） */
  outputColumnName?: string;
  /** 原始批量推理結果的列名（parquet中的列名） */
  originOutputColumnName?: string;
  /** 批量推理任務的id */
  batchInferTaskID?: Int64;
  /** 批量推理任務的狀態 */
  batchInferTaskStatus?: string;
  /** 批量推理輸入數據的其中一列列名，這列記錄了數據中每一行的唯一id */
  ...
  /** 火山項目名，用於記錄用戶託管的字節雲方舟賬號的項目名稱 */
  volcEngineProjectName?: string;
  /** 評測集中的圖片上傳到hdfs的地址（文件夾） */
...
export interface OfflineEvalTaskModel {
  ...
  /** 被評測的模型（當source是OpenSource/Ark基礎模型時，模型信息寫在這裏） */
  foundationModel?: model.SftTaskFoundationModel;
  /** 被評測的模型的標識（此時模型是sft產物，identification是方舟custom_id / model_version，且會記錄foundation model） */
  ...
  /** 被評測的模型文件的hdfs地址（當source是seed/sft/OpenSource時，模型hdfs地址寫在這裏） */
  modelAddress?: string;
