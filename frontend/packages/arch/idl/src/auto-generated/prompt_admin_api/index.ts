...
export default class PromptAdminApiService<T> {
   ...
   *
   * 因爲 AGW 中get 入參只支持逗號分割，但是前端請求是roleIds[]=1&roleIds[]=2，所以這裏只能用 post 方法
   */
  GetModelMetaList(
   ...
   *
   * GetPermModuleListResp GetPermModuleList(1:GetPermModuleListReq req)(api.post = '/api/admin/v1/perm/module_list', api.category = "權限管理") // 用戶有權限的模塊
   *
   * 獲取模型管理擁有的角色權限以及用戶當前有的權限
   */
  GetRoleListForModel(
