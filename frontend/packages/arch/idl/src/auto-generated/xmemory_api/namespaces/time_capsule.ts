...
export interface SearchItem {
  /** 搜索內容，可以是用戶輸入的原始query,也可以是需要進行查詢的關鍵詞 */
  query: string;
  /** 發起搜索時的位置信息 */
...
export interface TimeCapsuleSearchItemsRequest {
  ...
  /** TimeCapsule召回的最大長度限制，由上游copilot計算之後傳遞進來 */
  max_length?: Int64;
  /** multi-agent模式下，當前引用的bot_id */
  ref_bot_id?: string;
  ext?: Record<string, string>;
...
export interface TimeCapsuleSearchRequest {
  ...
  /** TimeCapsule召回的最大長度限制，由上游copilot計算之後傳遞進來 */
  max_length?: Int64;
  /** 默認是false，當爲true的時候，可以專門針對一些bot，不返回TimeCapsule召回結果。最開始主要是爲了一方應用而設計 */
  skip_block?: boolean;
  ...
  /** multi-agent模式下，當前引用的bot_id */
  ref_bot_id?: string;
  /** 支持通用透傳協議，可直接與最上游業務方和其他Hook接入方對接 */
  ext?: Record<string, string>;
