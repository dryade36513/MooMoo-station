...
export default class ProductApiService<T> {
   ...
   * 外部開放平臺接口相關
   *
   * 到其他平臺註冊服務的時候需要驗證，這個接口專門用來做驗證服務的工作
   */
  DouyinRegister(
   ...
   *
   * 圖片上傳採用 imageX 建議的方式，客戶端上傳：https://cloud-boe.bytedance.net/docs/imagex/wiki/ViwJdZcMxor0QzxGANlcwmXdncg?x-resource-account=boe&x-bc-region-id=bytedance
   *
   * 需要下面兩個接口：獲取臨時上傳 token；獲取圖片 url
   */
  PublicGetImageURL(
   ...
   *
   * 商品價格區間（插件商業化需求引入 - 上架時只能按 Coze 定義的價格區間定價）
   */
