...
當前實體類型 */
  ...
  entity_types?: Array<product_common.ProductEntityType>;
  /** deprecated：廢棄，採用 paid_type 參數； true = 篩選免費的；false = 篩選付費的；不傳則不區分免費和付費 */
  is_free?: boolean;
  /** 插件類型 */
...
export interface GetProductManageListRequest {
  ...
  /** 商品類型 */
  entity_types?: Array<product_common.ProductEntityType>;
  /** 組織ID，組織賬號下訪問時傳遞，用於權限控制，組織管理員可見組織內所有商品 */
...
export interface PluginBenefitReportConfig {
  /** 插件的調用配置，標記如何評估接口調用成功，成功的話才進行用量上報
回包中的 header 裏該 key 的值爲 true 時，認爲調用成功 */
  header?: string;
  /** 字段名 —— 成功值，接口返回參數該字段值等於成功值時，認爲調用成功 */
