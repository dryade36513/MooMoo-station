...
export interface CreateRole {
  name: string;
  role_type: RoleType;
  /** 3: required Visibility visibility, // 廢棄
廢棄,使用下面的participant_id */
...
export interface GetSocietyMessageListRequest {
  process_id: string;
  agent_id: string;
  /** 左開區間，可以用於切換視角後收到第一條推送消息時，傳最後一條消息的task_id，查詢結果直接加到消息列表後面，不用整體刷新了 */
  before_task_id?: string;
  /** 右開區間，倒序往前加載，最前一條消息的task_id，爲空則從最新的消息開始 */
...
export interface GetSocietyMetaDetailResponse {
  meta: Meta;
  role_list: Array<Role>;
  /** 3: optional ProcessDetail latest_process, // 廢棄 */
...
export interface Role {
  ...
  name: string;
  role_type: RoleType;
  /** 4: required Visibility visibility, // 廢棄
5: required i64 preset_bot_id (api.js_conv='true' agw.js_conv="str"), // 廢棄,使用下方的participant_id */
  nickname: string;
  description: string;
  meta_id: string;
  space_id: string;
...
export interface UpdateRole {
  ...
  role_type?: RoleType;
  visibility?: bot_common.SocietyVisibility;
  /** 5: optional i64 preset_bot_id (api.js_conv='true' agw.js_conv="str"), //廢棄,使用下面的participant_id */
  nickname?: string;
  description?: string;
  /** 區分是哪個渠道的bot，store還是teamspace */
  participant_source?: ParticipantSource;
...
export interface UserAgentReplyRequest {
  ...
  /** 用戶回覆內容 */
  content: string;
  /** 前置消息society_state裏的task_id */
  task_id: string;
  Base?: base.Base;
}

export interface UserAgentReplyResponse {
  /** true代表當前不應該有用戶回覆，前端主動刷新？重置一下？ */
...
/** 自己定義的WorkflowDetail，跟數據庫裏存的結構對應 */
