    ...
    <>
      ...
      {(projectId || IS_BOT_OP) && !IS_OPEN_SOURCE ? (
        <RadioSetterField
          ...
          options={{
            ...
            options: [
              {
                label: I18n.t('worklfow_start_basic_setting', {}, '基礎設置'),
        ...
        <>
          ...
          {isChatflow && FLAGS['bot.automation.message_auto_write'] ? (
            <Section title={I18n.t('basic_setting')} tooltip="">
              <SwitchField
                customLabel={I18n.t('workflow_250407_001', {}, '消息自動寫入')}
      ...
      {tabName === TriggerForm.Tab.Trigger ? (
        <>
          <>
            <Section
              title={I18n.t('workflow_start_trigger_setting', {}, '觸發器設置')}
              tooltip={I18n.t(
                'workflow_start_trigger_setting_tooltips',
                {},
                '選擇工作流的啓動方式，可以通過預設時間或自定義時間來觸發工作流的啓動',
              )}
              actions={[
                triggerIsOpen ? (
            ...
            {triggerIsOpen ? (
              <Section title={I18n.t('parameters', {}, '輸入')} actions={[]}>
                <div className="flex flex-col gap-[8px]">
                  <FixedInputParameter
