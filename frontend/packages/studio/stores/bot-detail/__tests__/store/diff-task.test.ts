...
describe('diff-task store', () => {
  ...
  test('setDiffTaskByImmer 應該正確更新狀態', () => {
  ...
  test('enterDiffMode 應該正確設置 prompt 類型的 diff 任務', () => {
  ...
  test('enterDiffMode 應該能處理非 prompt 類型的 diff 任務', () => {
    const { enterDiffMode } = useDiffTaskStore.getState();
  ...
  test('exitDiffMode 應該調用 clear 方法', () => {
