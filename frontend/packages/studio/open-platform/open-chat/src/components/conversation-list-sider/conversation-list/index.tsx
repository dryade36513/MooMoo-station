    ...
    const handleCreateConversation = async () => {
      ...
      try {
        const res = await cozeApi?.conversations.create({
          bot_id: botId,
          // @ts-expect-error: 有這個屬性，但是 openapi 沒有暴露
        ...
        if (res?.id) {
          ...
          Toast.info({
            content: I18n.t('web_sdk_create_conversation', {}, '已創建新會話'),
            showClose: false,
          });
        }
      } catch (error) {
      ...
      <div className={s.conversations}>
        <div className={s['conversations-header']}>
          <span className={s['conversations-header-title']}>
            {I18n.t('web_sdk_conversation_history', {}, '會話歷史')}
          </span>
          <IconButton
            color="secondary"
            onClick={() => {
        ...
        <Button
          ...
          loading={addLoading}
        >
          {I18n.t('web_sdk_add_new_conversation', {}, '創建新會話')}
        </Button>
