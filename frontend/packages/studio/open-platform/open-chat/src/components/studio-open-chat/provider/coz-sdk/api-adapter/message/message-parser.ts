...
>;
// 消息解析，主要用於從服務端獲取到消息後，解析成coze能適配的數據結構
enum ChunkEvent {
  ERROR = 'error',
  DONE = 'done',
...
export class MessageParser {
  private seqNo = 0; //標識消息的序號
  private indexNo = 0; //標識類型的序號
  ...
  private createAckMessage(data: string) {
    ...
    // @ts-expect-error -- linter-disable-autofix, 新添加參數，sdk中還未支持到
    ...
    return {
      ...
      data: {
        ...
        message: {
          ...
          role: 'user',
          // @ts-expect-error -- linter-disable-autofix, 新添加參數，sdk中還未支持到
          section_id: dataValue?.section_id || this.sectionId, //todo 添加代碼
