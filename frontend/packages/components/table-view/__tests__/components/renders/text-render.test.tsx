...
describe('TextRender', () => {
  ...
  test('應該在可編輯模式下正確渲染', () => {
    ...
    fireEvent.click(screen.getByText('測試文本'));

    // Verify that the TextArea is visible
    expect(screen.getByTestId('text-area')).toBeInTheDocument();
    expect(screen.getByTestId('text-area')).toHaveValue('測試文本');
  });
  ...
  test('應該在 isEditing 爲 true 時直接進入編輯模式', () => {
    render(
      <TextRender
        value="測試文本"
        record={mockRecord}
  ...
  test('應該在 isEditing 從 true 變爲 undefined 時退出編輯模式', async () => {
