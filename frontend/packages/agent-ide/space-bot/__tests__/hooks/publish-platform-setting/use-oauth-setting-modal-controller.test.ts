...
vi.mock('@coze-arch/bot-api', () => ({
  connectorApi: {
    GetOauthConfigSchema: vi.fn().mockResolvedValue({
      oauth_schema: {
        title_text: '填寫自定義渠道 oauth 配置信息',
        start_text:
          '按照[**自定義渠道配置流程**](https://www.coze.cn/docs/guides/isv_connector)配置並註冊自定義渠道，支持 Bot 發佈至自定義渠道',
        schema_area: {
          schema_list: [
            {
              type: 'string',
            ...
            },
            {
              ...
              rules: [
                {
                  message: '獲取 token 地址 必填',
                  required: true,
                },
              ],
              placeholder: '請輸入獲取 token 地址',
...
describe('useOauthSettingModalController', () => {
  it('useOauthSettingModalController fetch form config should be right', async () => {
    ...
    expect(result.current.oauthModalTitle).toBe('填寫 oauth 配置信息');
  });
