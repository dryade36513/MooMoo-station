...
var channel2GroupVariableInfo = map[project_memory.VariableChannel]project_memory.GroupVariableInfo{
	project_memory.VariableChannel_APP: {
		GroupName:    "應用變量",
		GroupDesc:    "用於配置應用中多處開發場景需要訪問的數據，每次新請求均會初始化爲默認值。",
		GroupExtDesc: "",
		IsReadOnly:   false,
	...
	project_memory.VariableChannel_Custom: {
		GroupName:    "用戶變量",
		GroupDesc:    "用於存儲每個用戶使用項目過程中，需要持久化存儲和讀取的數據，如用戶的語言偏好、個性化設置等。",
		GroupExtDesc: "",
		IsReadOnly:   false,
		SubGroupList: []*project_memory.GroupVariableInfo{},
		VarInfoList:  []*project_memory.Variable{},
	...
	project_memory.VariableChannel_System: {
		GroupName:    "系統變量",
		GroupDesc:    "可選擇開啓你需要獲取的，系統在用戶在請求自動產生的數據，僅可讀不可修改。如用於通過ID識別用戶或處理某些渠道特有的功能。",
		GroupExtDesc: "",
		IsReadOnly:   true,
		SubGroupList: []*project_memory.GroupVariableInfo{},
		VarInfoList:  []*project_memory.Variable{},
