...
// DefaultParamSource 表示默認參數的設置來源
type DefaultParamSource int32

const (
	DefaultParamSource_Input    DefaultParamSource = 0 // 默認用戶輸入
	DefaultParamSource_Variable DefaultParamSource = 1 // 引用變量
)

// AssistParameterType 表示輔助參數類型
...
type Parameter struct {
	...
	SubType            string               `json:"subType,omitempty"`    // 如果Type是數組，則有subtype
	FromNodeId         *string              `json:"fromNodeId,omitempty"` // 如果入參的值是引用的則有fromNodeId
	FromOutput         []string             `json:"fromOutput,omitempty"` // 具體引用哪個節點的key
	Value              *string              `json:"value,omitempty"`      // 如果入參是用戶手輸 就放這裏
	...
	DefaultParamSource *DefaultParamSource  `json:"defaultParamSource,omitempty"` // 默認入參的設置來源
