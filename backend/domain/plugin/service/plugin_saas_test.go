...
func TestSearchSaasPluginResponse_JSONUnmarshal(t *testing.T) {
	...
	assert.Equal(t, "ppe_test_官方付費", metaInfo.Name)
...
func TestJsonSchemaTypeUnmarshaling(t *testing.T) {
	...
	jsonData := `{
		"plugins": [{
			"tools": [{
				...
				"description": "當你需要獲取網頁、pdf、doc、docx、xlsx、csv、text 內容時，使用此工具",
				"name": "LinkReaderPlugin",
				"inputSchema": {
					"required": ["url"],
					"properties": {
						"need_image_url": {
							"description": "是否需要返回圖片url",
							"type": "boolean"
						},
						"url": {
							"description": "網頁url、pdf url、docx url、csv url、 xlsx url。",
							"type": "string"
						}
					},
					"type": "object"
	...
	assert.Equal(t, "網頁url、pdf url、docx url、csv url、 xlsx url。", urlProp.Description)
	...
	assert.Equal(t, "是否需要返回圖片url", needImageProp.Description)
...
func TestConvertFromJsonSchemaWithFixedType(t *testing.T) {
	...
	assert.Equal(t, "圖片的url", urlParam.Desc)
	...
	assert.Equal(t, "是否需要返回圖片url", imageParam.Desc)
...
func TestBatchGetSaasPluginToolsInfoIntegration(t *testing.T) {
	...
	respData := `{
		"plugins": [{
			"tools": [{
				"tool_id": "7379227817307029513",
				"description": "當你需要獲取網頁、pdf、doc、docx、xlsx、csv、text 內容時，使用此工具，可以獲取url鏈接下的標題和內容。由於個別網站自身站點限制，無法獲取網頁內容。",
				"name": "LinkReaderPlugin",
				"inputSchema": {
					"required": ["url"],
					"properties": {
	...
	assert.Equal(t, pluginCommon.ParameterType_String, urlParam.Type)
	assert.Equal(t, pluginCommon.ParameterLocation_Body, urlParam.Location)
	assert.Equal(t, "網頁url、pdf url、docx url、csv url、 xlsx url。", urlParam.Desc)
