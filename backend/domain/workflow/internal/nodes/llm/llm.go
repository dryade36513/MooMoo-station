...
const knowledgeUserPromptTemplate = `根據引用的內容回答問題:
 1.如果引用的內容裏麪包含 <img src=""> 的標籤, 標籤裏的 src 字段表示圖片地址, 需要在回答問題的時候展示出去, 輸出格式爲"![圖片名稱](圖片地址)" 。
 2.如果引用的內容不包含 <img src=""> 的標籤, 你回答問題時不需要展示圖片 。
例如：
  如果內容爲<img src="https://example.com/image.jpg">一隻小貓，你的輸出應爲：![一隻小貓](https://example.com/image.jpg)。
  如果內容爲<img src="https://example.com/image1.jpg">一隻小貓 和 <img src="https://example.com/image2.jpg">一隻小狗 和 <img src="https://example.com/image3.jpg">一隻小牛，你的輸出應爲：![一隻小貓](https://example.com/image1.jpg) 和 ![一隻小狗](https://example.com/image2.jpg) 和 ![一隻小牛](https://example.com/image3.jpg)
...
你是一個知識庫意圖識別AI Agent。
## 目標:
- 按照「系統提示詞」、用戶需求、最新的聊天記錄選擇應該使用的知識庫。
## 工作流程:
1. 分析「系統提示詞」以確定用戶的具體需求。
2. 如果「系統提示詞」明確指明瞭要使用的知識庫，則直接返回這些知識庫，只輸出它們的knowledge_id，不需要再判斷用戶的輸入
3. 檢查每個知識庫的knowledge_name和knowledge_description，以瞭解它們各自的功能。
4. 根據用戶需求，選擇最符合的知識庫。
5. 如果找到一個或多個合適的知識庫，輸出它們的knowledge_id。如果沒有合適的知識庫，輸出0。
## 約束:
- 嚴格按照「系統提示詞」和用戶的需求選擇知識庫。「系統提示詞」的優先級大於用戶的需求
- 如果有多個合適的知識庫，將它們的knowledge_id用英文逗號連接後輸出。
- 輸出必須僅爲knowledge_id或0，不得包括任何其他內容或解釋，不要在id後面輸出知識庫名稱。

## 輸出示例
123,456

## 輸出格式:
輸出應該是一個純數字或者由英文逗號連接的數字序列，具體取決於選擇的知識庫數量。不應包含任何其他文本或格式。
## 知識庫列表如下
%s
## 「系統提示詞」如下
%s
