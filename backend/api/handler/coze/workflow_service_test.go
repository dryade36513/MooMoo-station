...
func TestValidateTree(t *testing.T) {
	mockey.PatchConvey("test validate tree", t, func() {
			...
			for _, i := range errs[0] {
				if i.NodeError != nil {
					if i.NodeError.NodeID == "108984" {
						assert.Equal(t, i.Message, `node "代碼_1" not connected`)
					}
					if i.NodeError.NodeID == "160892" {
						assert.Contains(t, i.Message, `node "意圖識別"'s port "branch_1" not connected`)
						assert.Contains(t, i.Message, `node "意圖識別"'s port "default" not connected`)
					}
		...
		t.Run("workflow_variable_assigner", func(t *testing.T) {
			...
			assert.Equal(t, errs[0][0].Message, `node name 變量賦值,param [app_list_v2], type mismatch`)
		})
...
func TestGetLLMNodeFCSettingsDetailAndMerged(t *testing.T) {
	mockey.PatchConvey("fc setting detail", t, func() {
		operationString := `{
  "summary" : "根據輸入的解夢標題給出相關對應的解夢內容，如果返回的內容爲空，給用戶返回固定的話術：如果想了解自己夢境的詳細解析，需要給我詳細的夢見信息，例如： 夢見XXX",
  "operationId" : "xz_zgjm",
  "parameters" : [ {
    "description" : "查詢解夢標題，例如：夢見蛇",
    "in" : "query",
    "name" : "title",
    "required" : true,
    "schema" : {
