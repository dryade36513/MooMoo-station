...
export const loadConfigFromFile = async (
  ...
  try {
    ...
    console.warn(`配置文件加載失敗: ${configPath}`, error);
    return {};
  }
};
...
export const validateConfig = (
  ...
  if (!config.translation.accessKeyId) {
    errors.push(
      '火山引擎 Access Key ID 未設置，請通過環境變量VOLC_ACCESS_KEY_ID或--access-key-id參數提供',
    );
  }
  ...
  if (!config.translation.secretAccessKey) {
    errors.push(
      '火山引擎 Secret Access Key 未設置，請通過環境變量VOLC_SECRET_ACCESS_KEY或--secret-access-key參數提供',
    );
  }
  ...
  if (!validRegions.includes(config.translation.region)) {
    console.warn(
      `未知的區域: ${config.translation.region}，建議使用: ${validRegions.join(
        ', ',
      )}`,
    );
  }
  ...
  if (
    ...
    errors.push('超時時間應該在1000-300000毫秒之間');
  }
...
export const printConfigInfo = (
  ...
  console.log('🔧 當前配置:');
  console.log(`  區域: ${config.translation.region}`);
  console.log(`  源語言: ${config.translation.sourceLanguage}`);
  console.log(`  目標語言: ${config.translation.targetLanguage}`);
  console.log(`  併發數: ${config.translation.concurrency}`);
  console.log(`  重試次數: ${config.translation.maxRetries}`);
  console.log(`  輸出格式: ${config.processing.outputFormat}`);

  if (verbose) {
    console.log(
      `  Access Key ID: ${
        config.translation.accessKeyId ? '已設置' : '未設置'
      }`,
    );
    console.log(
      `  Secret Access Key: ${
        config.translation.secretAccessKey ? '已設置' : '未設置'
      }`,
    );
    console.log(`  超時時間: ${config.translation.timeout}ms`);
    console.log(
      `  默認擴展名: ${config.processing.defaultExtensions.join(', ')}`,
    );
    console.log(`  忽略模式: ${config.git.ignorePatterns.join(', ')}`);
    console.log(
      `  包含未跟蹤文件: ${config.git.includeUntracked ? '是' : '否'}`,
    );
  }
};
