...
處理成功: ${stats.processedFiles}
跳過文件: ${stats.skippedFiles}
翻譯註釋: ${stats.translatedComments}
錯誤數量: ${stats.errors.length}
成功率: ${successRate}%
處理時間: ${duration.toFixed(2)}秒
`;
...
export class ProgressDisplay {
  ...
  update(current: number, currentFile?: string): void {
    ...
    let line = `進度: ${current}/${
      this.total
    } (${percentage}%) | 耗時: ${elapsed.toFixed(1)}s`;

    if (eta > 0) {
      line += ` | 預計剩餘: ${eta.toFixed(1)}s`;
    }
