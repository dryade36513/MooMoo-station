...
const parseSingleLineComments = (
  ...
  if (lines.length > maxLines) {
    console.warn(`⚠️  文件行數過多 (${lines.length}行)，跳過單行註釋解析`);
    return comments;
  }
  ...
  lines.forEach((line, index) => {
    ...
    while ((match = pattern.exec(line)) !== null) {
      ...
      if (matchCount > maxMatches) {
        console.warn(
          `⚠️  單行匹配次數過多，中斷處理: ${line.substring(0, 50)}...`,
        );
        break;
      }
...
const parseMultiLineComments = (
  ...
  const maxLines = 5000; // Down to 5000 lines
  if (lines.length > maxLines) {
    console.warn(`⚠️  文件行數過多 (${lines.length}行)，跳過多行註釋解析`);
    return comments;
  }
...
export const detectChineseInFile = (file: SourceFile): ChineseComment[] => {
  try {
    ...
    if (file.content.length > 500000) {
      // 500KB
      console.warn(
        `⚠️  跳過大文件: ${file.path} (${file.content.length} 字符)`,
      );
      return [];
    }
    ...
    if (lines.length > 10000) {
      console.warn(`⚠️  跳過多行文件: ${file.path} (${lines.length} 行)`);
      return [];
    }
    ...
    console.error(`❌ 文件處理失敗: ${file.path} - ${error}`);
    return [];
  }
};
...
export const detectChineseInFiles = (
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    console.log(`🔍 檢測進度: ${i + 1}/${files.length} (當前: ${fileName})`);
    ...
    try {
      ...
      console.log(
        `✅ 完成: ${fileName} (找到 ${chineseComments.length} 條中文註釋)`,
      );
    } catch (error) {
      console.error(`❌ 處理文件失敗: ${fileName} - ${error}`);
