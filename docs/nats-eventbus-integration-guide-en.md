...
2. **High Performance**: Provides low-latency, high-throughput messaging that can support MooMoo Station's large-scale concurrent Agent execution
...
NATS supports two messaging modes in MooMoo Station:
...
Configure NATS in MooMoo Station application through environment variables:
...
NATS as MooMoo Station's EventBus solution provides lightweight, high-performance, and easy-to-deploy messaging capabilities. Through JetStream extensions, NATS can also provide enterprise-grade message persistence and stream processing functionality.
